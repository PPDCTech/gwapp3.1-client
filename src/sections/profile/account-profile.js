...
import { updateProfilePic, updateSignaturePic } from "src/services/api/users.api";
...
export const AccountProfile = () => {
  ...
  const handleSignatureUpload = async () => {
    try {
      ...
      const update_response = await updateSignaturePic(user._id, updatedSignatureUrl);
    ...
    <Card>
      <CardContent>
        ...
        >
          ...
          <Typography variant="body2">Role: {capitalizeFirstLetter(user?.role)}</Typography>
        </Box>
      </CardContent>
      ...
      {image && (
        <Box sx={{ display: "flex", justifyContent: "space-around" }}>
          ...
          />
          <Button variant="text" color="success" onClick={handleImageUpload} disabled={saving}>
      ...
      <Container>
        <Box sx={{ display: "flex", justifyContent: "center", flexDirection: "column" }}></Box>
