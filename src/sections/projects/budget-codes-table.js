		...
		<>
			<Grid container spacing={2}>
				...
				<Grid item xs={12} md={6} sx={{ display: "flex", alignItems: "center" }}>
					<AddBudgetCodeModal onAddNew={handleBudgetCode} user={user} />
				</Grid>
			</Grid>
			{budgetCodes && budgetCodes.length > 0 ? (
				<>
					{budgetCodes.filter((code) =>
						...
						<Typography sx={{ mt: 2 }}>
							No budget codes found for this query.
						</Typography>
					)}
					<TableContainer>
						<Table>
									...
									.map((code) => (
										...
										>
											...
											<TableCell>
												{editId === code._id ? (
													<IconButton
														onClick={() => handleSaveClick(code._id)}
														aria-label="save"
													>
