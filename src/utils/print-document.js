...
export const printDocument = async (req) => {
  ...
  let userSignature = await tobase64(req.user.signatureUrl);
  ...
  if (req.type === "Fund Req") {
    ref =
      formatDateHyphen(req.date) +
      req.projectId.substr(0, 10) +
      "-" +
      req.projectName.substr(0, 3).toUpperCase();

    let budgetHolderSig = await tobase64(req.holderCheck.signatureUrl);
    let checkedSignature = await tobase64(req.checkedBy.signatureUrl);
